...
passRecov.requestCode = async (req, res) => {
    ...
    try {
        ...
        res.cookie('recoveryToken', token, {httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'strict', maxAge: 60 * 60 * 1000})

        await sendEmail({
            to: email,
            subject: `Password Recovery Code ${code}`,
...
passRecov.verifyCode = async (req, res) => {
    ...
    try {
        ...
        res.cookie('recoveryToken', newToken, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'strict', maxAge: 60 * 60 * 1000})
